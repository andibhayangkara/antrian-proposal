...
Route::group(['middleware' => 'install'], function() {
	...
	Route::group(['middleware' => 'auth:users'], function() {
        ...
        Route::get('calls/reset/{dept}', function($id){
            $call = DB::table('calls')->where('department_id', $id)->get();
            for ($i=0; $i < count($call) ; $i++) {
                DB::table('calls')->where('department_id', $id)->where('id', $call[$i]->id)->delete(); 
            }
            $data = DB::table('queues')->where('department_id', $id)->get();
            for ($i=0; $i < count($data) ; $i++) {
                DB::table('queues')->where('department_id', $id)->where('id', $data[$i]->id)->delete(); 
            }
            flash()->success('Antrian direset');
